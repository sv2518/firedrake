        ...
        stages {
          ...
          stage('Build') {
            ...
            steps {
              ...
              dir('tmp') {
                timestamps {
                  sh '../scripts/firedrake-install $COMPLEX --tinyasm --disable-ssh --minimal-petsc --slepc --documentation-dependencies --install thetis --install gusto --install icepack --install irksome --install femlium --no-package-manager --package-branch tsfc ReubenHill/0d-mesh-interpolation-hack --package-branch FInAT ReubenHill/0d-mesh-interpolation-hack || (cat firedrake-install.log && /bin/false)'
                }
              }
            }
          }
