...
def _push_mul_transpose(expr, self, state):
    ...
    """
    if expr.rank == 2:
        transposed_state = ActionBag(Transpose(state.coeff), state.pick_op^1)   # switch the multiplication order with pick_op 
        pushed_expr = self(*expr.children, transposed_state)                    # push mul into A with new state
        return self(Transpose(pushed_expr), state)                              # transpose the end result
    else:
        return expr
...
def _push_mul_mul(expr, self, state):
    ...
    if expr.rank == 1:
        ...
        prio_child, = tuple(filter(lambda child: child.rank == 1, expr.children))   # 1a) (A2+A3)*y             1b) (y.T*A0+y.T*A1)
        ...
        prio_child = expr.children[state.pick_op]                                   #                                                           2c) C
        pick_op = state.pick_op
    other_child = expr.children[pick_op^1]                                          # 1a) A0*A1                     1b) B                       2c)A.solve(B.T)
    if state.coeff and expr.rank == 1:
        ...
        # Optimise child of rank 1 first but do not use result as coefficient.
        coeff = self(prio_child, ActionBag(None, pick_op))                          # 1a) (A2*y+A3*y)               1b) y.T*A0+y.T*A1)         
        pushed_other_child = self(other_child, ActionBag(state.coeff, pick_op))  
        return Mul(pushed_other_child, prio_child) if pick_op else Mul(prio_child, pushed_other_child)            # 1b)(y.T*A0+y.T*A1)*(B*y)    
    else:
        # Optimise child of rank 1 first and use result as coefficient
        coeff = self(prio_child, state)                                             #                                                           2c)y.T*C
        return self(other_child, ActionBag(coeff, pick_op))                         # 1a) push_mul(A0*A1,(A2*y+A3*y))                           2c)push_mul(A.solve(B), y.T*C)
