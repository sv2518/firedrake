...
def _push_mul_inverse(expr, self, state):
    ...
    if expr.diagonal:
        ...
        if (self.action and state.coeff                                              # in matrix-free mode
            ...
            mat = child
            coeff = state.coeff
        expr.preconditioner = preconditioner_l
        expr = (Solve(mat, coeff, preconditioner=preconditioner_l, **expr.ctx) if state.pick_op
                else Transpose(Solve(Transpose(mat), Transpose(coeff), preconditioner=preconditioner_l, **expr.ctx)))
